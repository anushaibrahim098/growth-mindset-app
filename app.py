...
if not st.session_state.tasks:
   st.info("No task addad yet start by adding a task from the sidebar!")
else:
    for index, task in enumerate(st.session_state.tasks):
        coll , col2 , col3 = st.columns([0.7,0.15,0.15])

        # mark as completed
        completed = col1 = st.columns(f"**{task['task']}**",task["completed"],key=f"check_{index}")
...
for index, task in enumerate(st.session_state.tasks):
    ...
    if col3.button("Deleted", key=f"delete_{index}"):
        del st.session_state.tasks[index]
        st.experimental_rerun()
            
# clear all tasks
if st.button("clear all tasks"):
   st.button_state.tasks = []
